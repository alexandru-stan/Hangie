...
using namespace std;
     ...
     int GetTodaysWord::open_db_connection(sqlite3 **db) {
         ...
         int connection_fail = sqlite3_open("..\\..\\..\\data\\sqlite\\hangie.db", db);
         return connection_fail;
     }
     
     string GetTodaysWord::fetch_db_word(sqlite3** db) {

         const char* sql = "SELECT word FROM words WHERE used = false AND word LIKE '%rbol' ORDER BY random() LIMIT 1";
         sqlite3_stmt* stmt;
